...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.l = tk.Label(textvar=self.var, bg='white', width=50, height=3)  # 参数textvar不同于text,bg是backgroud
        self.l.pack()
...
#        self.after(500, func=self.train)
    ...
    def on_button_move(self, event):
        ...
        if self.previous_x is None and self.previous_y is None:
            ...
            self.draw.line([self.previous_x, self.previous_y, event.x, event.y],fill='black',width=5)
            self.previous_x = event.x
            self.previous_y = event.y
#            kernel_size = 5
